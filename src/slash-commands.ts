...
export class SlashCommands {
  ...
  public generateHelpText(): string {
    ...
    const planningPhase = allCommands.filter(cmd =>
      cmd.command === '/plan_project' ||
      cmd.command === '/refine_requirements' ||
      cmd.command === '/analyze_project'
    );

    const specificationPhase = allCommands.filter(cmd =>
      cmd.command === '/generate_specification' ||
      cmd.command === '/enhance_specification'
    );

    const implementationPhase = allCommands.filter(cmd =>
      cmd.command === '/plan_implementation' ||
      cmd.command === '/plan_enhancement'
    );

    const qualityPhase = allCommands.filter(cmd =>
      cmd.command === '/generate_tests' ||
      cmd.command === '/generate_enhancement_tests'
    );

    const documentationPhase = allCommands.filter(cmd =>
      cmd.command === '/create_documentation'
    );

    const reviewPhase = allCommands.filter(cmd =>
      cmd.command === '/final_review'
    );

    const implementationStartPhase = allCommands.filter(cmd =>
      cmd.command === '/start_implementation'
    );

    const utilityPhase = allCommands.filter(cmd =>
      cmd.command === '/help' ||
