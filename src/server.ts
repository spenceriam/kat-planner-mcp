  ...
  private registerTools(): void {
    ...
    this.server.registerTool('refinement_tool', {
      ...
      inputSchema: {
        ...
        currentStage: z.enum(['initial', 'clarifying', 'summarizing']).describe('Current stage - MUST match actual workflow progress'),
        answers: z.record(z.string()).optional().describe('User answers from previous stage - REQUIRED if not first call'),
      },
    ...
    this.server.registerTool('sdd_gen', {
      ...
      inputSchema: {
        refinedSpec: z.string().describe('The approved and refined project specification WITH explicit user approval'),
